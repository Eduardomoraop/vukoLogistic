...
const initialMovements = [
  { id: 1, fecha: "2026-07-02 12:30", codigo_referencia: "REF-4502", tipo: "ENTRADA", cantidad: 50, operador: "Carlos Gómez", notas: "Compra de stock regular" },
  { id: 2, fecha: "2026-07-02 11:15", codigo_referencia: "REF-1092", tipo: "SALIDA", cantidad: 2, operador: "Ana Ruiz", notas: "Despacho cliente VukoStore" },
  { id: 3, fecha: "2026-07-02 10:00", codigo_referencia: "REF-8831", tipo: "ENTRADA", cantidad: 10, operador: "Felipe Marcos", notas: "Devolución de lote de prueba" },
  { id: 4, fecha: "2026-07-01 16:45", codigo_referencia: "REF-7742", tipo: "SALIDA", cantidad: 35, operador: "Carlos Gómez", notas: "Orden de envío #9923" },
  { id: 5, fecha: "2026-07-01 09:30", codigo_referencia: "REF-0021", tipo: "ENTRADA", cantidad: 100, operador: "Laura Milla", notas: "Importación aduana Lote C" },
];
...
const Movimientos = () => {
  ...
  const handleAddMovement = (e) => {
    ...
    if (!newMovement.codigo_referencia || isNaN(qty) || !newMovement.operador) return;

    const dateNow = new Date();
    const formattedDate = dateNow.toISOString().slice(0,10) + " " + dateNow.toTimeString().slice(0,5);
  ...
  return (
    <div className="p-8 bg-slate-50 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
        <div>
          <h2 className="text-3xl font-extrabold text-slate-800 tracking-tight">Kardex / Historial de Movimientos</h2>
          <p className="text-slate-600 mt-1">Bitácora completa de entradas, salidas y traspasos en tiempo real.</p>
        </div>
        <button
      ...
      <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center space-x-3">
          <label className="text-xs font-bold text-slate-500 uppercase tracking-wider">Tipo Movimiento:</label>
          <select
            value={filterType}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        {filteredMovements.length === 0 ? (
          <div className="text-center py-16 text-slate-500">No se encontraron movimientos.</div>
        ) : (
            ...
            <table className="min-w-full divide-y divide-slate-100">
              <thead className="bg-slate-50">
                <tr>
                  ...
                  <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Cantidad</th>
                  <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Operador</th>
              ...
              <tbody className="divide-y divide-slate-100 bg-white">
                {filteredMovements.map((mov) => (
                  <tr key={mov.id} className="hover:bg-slate-50/50 transition">
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500 font-medium">{mov.fecha}</td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-mono font-bold text-slate-700">{mov.codigo_referencia}</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-extrabold text-slate-900">{mov.cantidad} ud</td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-700 font-semibold">{mov.operador}</td>
                    <td className="px-6 py-4 text-sm text-slate-500">{mov.notas}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl border border-slate-200 overflow-hidden transform scale-100 transition-all">
            <div className="p-6 border-b text-white bg-teal-600 flex items-center justify-between">
              <h3 className="text-lg font-bold">Registrar Movimiento en Bitácora</h3>
            ...
            <form onSubmit={handleAddMovement} className="p-6 space-y-4">
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Referencia del Producto</label>
                <input
                  type="text"
                  placeholder="REF-4502"
                  required
                  value={newMovement.codigo_referencia}
                  onChange={(e) => setNewMovement({ ...newMovement, codigo_referencia: e.target.value })}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Tipo Movimiento</label>
                  <select
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Operador / Responsable</label>
                <input
                  type="text"
                  placeholder="Tu Nombre"
                  required
                  value={newMovement.operador}
                  onChange={(e) => setNewMovement({ ...newMovement, operador: e.target.value })}
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 uppercase tracking-wider mb-1">Observación / Notas</label>
                <input
                  type="text"
