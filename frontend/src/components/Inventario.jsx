...
const Inventario = () => {
  ...
  const typeBadge = (tipo) => {
    ...
    switch (tipo) {
      case "Pallet":
        return (
          <span
            className={`${base} bg-amber-50 text-amber-700 border border-amber-200/50`}
          >
            {tipo}
          </span>
        );
      case "Contenedor":
        return (
          <span
            className={`${base} bg-sky-50 text-sky-700 border border-sky-200/50`}
          >
            {tipo}
          </span>
        );
      default:
        return (
          <span
            className={`${base} bg-slate-50 text-slate-700 border border-slate-200/50`}
          >
            {tipo}
          </span>
        );
  ...
  const handleAddItem = (e) => {
    ...
    if (!newItem.codigo_referencia || !newItem.descripcion || isNaN(stock))
      return;
  ...
  const handleDeleteItem = (id) => {
    if (
      window.confirm(
        "¿Seguro que deseas eliminar esta referencia del inventario?",
      )
    ) {
  ...
  const filteredItems = items.filter((item) => {
    ...
    const matchesType =
      filterType === "todos" ||
      item.tipo_unidad.toLowerCase() === filterType.toLowerCase();
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-600 mt-1">
            Administra referencias, stock y ubicaciones de almacén.
          </p>
        </div>
        <button
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) =>
                      setNewItem({ ...newItem, tipo_unidad: e.target.value })
                    }
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setNewItem({ ...newItem, stock_actual: e.target.value })
                    }
