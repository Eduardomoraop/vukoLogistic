...
const mockItems = [
  {
    ...
    tipo_unidad: "Caja",
    stock_actual: 125,
    ubicacion_almacen: "Estantería A-3",
  },
  {
    id: 2,
    codigo_referencia: "REF-1092",
    descripcion: 'Monitores UltraWide 34"',
    tipo_unidad: "Contenedor",
    stock_actual: 6,
    ubicacion_almacen: "Muelle 1",
  },
  {
    id: 3,
    ...
    tipo_unidad: "Pallet",
    stock_actual: 8,
    ubicacion_almacen: "Zona B-Pallets",
  },
  {
    id: 4,
    codigo_referencia: "REF-0021",
    descripcion: "Cables HDMI 2.1 de Alta Velocidad",
    tipo_unidad: "Caja",
    stock_actual: 45,
    ubicacion_almacen: "Estantería A-4",
  },
  {
    id: 5,
...
const Dashboard = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const payload = {
      ...
      cantidad: qty,
      descripcion:
        form.descripcion || `${modalType.toUpperCase()} manual de stock`,
    };
    ...
    try {
      ...
      setMessage({
        type: "success",
        text: `Movimiento de ${modalType} registrado exitosamente en base de datos.`,
      });
      ...
      const exists = items.some(
        (item) => item.codigo_referencia === form.referencia,
      );
  ...
  return (
    <div className="p-8 bg-slate-50 min-h-screen">
      ...
      {message && (
        <div
          className={`mb-6 p-4 rounded-xl text-sm font-semibold border flex items-center justify-between transition-all shadow-sm ${
        ...
        >
          <div className="flex items-center space-x-2">
            <span
              className={`w-2.5 h-2.5 rounded-full ${message.type === "success" ? "bg-emerald-500" : "bg-amber-500"}`}
            ></span>
            <span>{message.text}</span>
          </div>
          <button
            onClick={() => setMessage(null)}
            className="hover:opacity-70 text-slate-400 font-bold ml-4"
          >
            ✕
          </button>
        </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-3">
          <div>
            ...
            <p className="text-slate-500 text-sm">
              Listado en tiempo real de unidades y referencias de inventario
            </p>
          </div>
          <button
            onClick={fetchItems}
      ...
      {showModal && (
        <div className="fixed inset-0 bg-slate-900/50 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-all">
          <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl border border-slate-200 overflow-hidden transform scale-100 transition-all">
            <div
              className={`p-6 border-b text-white flex items-center justify-between ${
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-4">
              <div>
                ...
                >
                  ...
                  {modalType === "entrada" && (
                    <option value="NUEVO">Crear Nueva Referencia...</option>
                  )}
                </select>
              </div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setForm({
                            ...form,
                            referenciaInput: e.target.value.toUpperCase(),
                          })
                        }
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setForm({ ...form, tipo_unidad: e.target.value })
                        }
