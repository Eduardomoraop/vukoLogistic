...
const Reportes = () => {
  ...
  const handleExport = (format) => {
    alert(
      `Exportando reporte de inventario en formato ${format.toUpperCase()}...`,
    );
  };
    ...
    <div className="p-8 bg-slate-50 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-8 gap-4">
        <div>
          ...
          <p className="text-slate-600 mt-1">
            Monitorea las tendencias de existencias y flujos de mercancía.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm">
          ...
          <p className="text-3xl font-extrabold text-slate-800 mt-2">
            {(
              (currentData.exits.reduce((a, b) => a + b, 0) /
                currentData.stockLevels[currentData.stockLevels.length - 1]) *
              100
            ).toFixed(1)}
            %
          </p>
          ...
          <div className="flex items-center justify-between mb-6">
            <div>
              <h3 className="text-lg font-bold text-slate-800">
                Tendencia de Almacén
              </h3>
              <p className="text-xs text-slate-500">
                Representación visual interactiva del volumen de stock
              </p>
            </div>
            ...
            {activeChart === "stock" ? (
              ...
              >
                ...
                <polygon
                  ...
                  points={`
                    ...
                    ${currentData.stockLevels
                      .map((val, idx) => {
                        const x =
                          (idx / (currentData.stockLevels.length - 1)) * 600;
                ...
                <polyline
                  ...
                  points={currentData.stockLevels
                    .map((val, idx) => {
                      const x =
                        (idx / (currentData.stockLevels.length - 1)) * 600;
                      const y = 180 - (val / maxStock) * 140;
              ...
              <div className="w-full h-full flex items-end justify-around pt-6">
                {currentData.labels.map((label, idx) => {
                  const entryHeight =
                    (currentData.entries[idx] / maxMove) * 140;
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 flex flex-col justify-between">
          <div>
            ...
            <p className="text-slate-500 text-sm mb-6">
              Genera informes auditables de inventarios, stock valorado y
              rotación listos para descarga.
            </p>
            ...
            <div className="space-y-3.5">
                ...
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    ...
                    <p className="text-slate-400 text-[10px]">
                      Kardex resumido del período actual
                    </p>
                  </div>
                </div>
                <svg
                  className="w-4 h-4 text-slate-400 group-hover:text-teal-600 transition"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2.5"
                  viewBox="0 0 24 24"
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h4 className="text-sm font-bold text-slate-800">
                      Base de Datos Stock (CSV)
                    </h4>
                    <p className="text-slate-400 text-[10px]">
                      Excel de existencias y ubicaciones
                    </p>
                  </div>
                </div>
                <svg
