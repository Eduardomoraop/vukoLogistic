...
const initialKpiData = [
  {
    id: 1,
    referencia: "REF-4502",
    descripcion: "Teclados Mecánicos RGB",
    tipo: "Caja",
    stock: 125,
    ubicacion: "Almacén A",
    valorUnitario: 45,
  ...
  },
  {
    id: 2,
    referencia: "REF-1092",
    descripcion: 'Monitores UltraWide 34"',
    tipo: "Contenedor",
    stock: 6,
    ubicacion: "Almacén Muelle",
    valorUnitario: 320,
    diasSinMovimiento: 12,
    salidasMes: 14,
  },
  {
    id: 3,
    referencia: "REF-8831",
    descripcion: "Sillas Ergonómicas Premium",
    tipo: "Pallet",
    stock: 8,
    ubicacion: "Almacén B",
    valorUnitario: 180,
  ...
  },
  {
    id: 4,
    referencia: "REF-0021",
    descripcion: "Cables HDMI 2.1 Alta Vel.",
    tipo: "Caja",
    stock: 45,
    ubicacion: "Almacén A",
    valorUnitario: 15,
...
const Kpis = () => {
  ...
  const itemsWithMetrics = initialKpiData.map((item) => {
    ...
    const rotacion =
      item.stock > 0 ? (item.salidasMes / item.stock).toFixed(2) : 0;
    ...
    const diasCobertura =
      salidasDiarias > 0 ? Math.round(item.stock / salidasDiarias) : 999;
  ...
  const groupedData = () => {
    ...
    Object.keys(groups).forEach((key) => {
      groups[key].diasSinMovPromedio = Math.round(
        groups[key].diasSinMovPromedio / groups[key].itemsCount,
      );
      groups[key].rotacionPromedio = (
        groups[key].salidasTotales / (groups[key].stockTotal || 1)
      ).toFixed(2);
    });
  ...
  const globalValorTotal = itemsWithMetrics.reduce(
    (sum, item) => sum + item.valorTotal,
    0,
  );
  const globalStockTotal = itemsWithMetrics.reduce(
    (sum, item) => sum + item.stock,
    0,
  );
  const stockCriticoCount = itemsWithMetrics.filter(
    (item) => item.diasCobertura <= 7,
  ).length;
    ...
    <div className="p-8 bg-slate-50 min-h-screen">
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-600 mt-1">
          Análisis de rendimiento, valoración financiera de stock y tablas
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm flex items-center justify-between">
          <div>
            <p className="text-slate-400 text-xs font-bold uppercase tracking-wider">
              Productos en Ruptura Crítica
            </p>
            <p className="text-3xl font-extrabold text-rose-600 mt-2">
              {stockCriticoCount}
            </p>
            <p className="text-xs text-slate-450 mt-1">
              Con menos de 7 días de cobertura estimados
            </p>
          </div>
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200/80 shadow-sm flex items-center justify-between">
          <div>
            ...
            <p className="text-3xl font-extrabold text-teal-600 mt-2">
              {(
                itemsWithMetrics.reduce((sum, i) => sum + i.salidasMes, 0) /
                globalStockTotal
              ).toFixed(2)}
              x
            </p>
            <p className="text-xs text-slate-450 mt-1">
              Frecuencia mensual de renovación de mercancías
            </p>
          </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6 gap-4">
          <div>
            <h3 className="text-lg font-bold text-slate-800">
              Matriz de Análisis Dinámico
            </h3>
            <p className="text-xs text-slate-500">
              Agrupa y ordena métricas clave agregadas para toma de decisiones
              tácticas.
            </p>
          </div>
          ...
          <div className="flex items-center space-x-3 bg-slate-100 p-1 rounded-xl self-start">
            <span className="text-[10px] font-extrabold text-slate-500 uppercase pl-2 tracking-wide">
              Agrupar por:
            </span>
            <button
          ...
          <table className="min-w-full divide-y divide-slate-100">
            <thead className="bg-slate-50">
              <tr>
                ...
                <th
                  ...
                  className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider cursor-pointer hover:bg-slate-100 select-none"
                >
                  Días Inactividad Promedio{" "}
                  {sortField === "diasSinMovPromedio"
                    ? sortAsc
                      ? "▲"
                      : "▼"
                    : ""}
            ...
            <tbody className="divide-y divide-slate-100 bg-white">
              {activeGrouped.map((group) => (
                ...
                >
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-extrabold text-slate-850">
                    {group.name}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-semibold text-slate-650">
                    {group.itemsCount} productos
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-slate-800">
                    {group.stockTotal} ud
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-extrabold text-slate-900">
                    ${group.valorTotal.toLocaleString("es-ES")} USD
                  </td>
