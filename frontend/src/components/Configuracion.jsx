...
const Configuracion = () => {
  const [supabaseUrl, setSupabaseUrl] = useState(
    "https://obtwjygpldxlyzqrkldj.supabase.co",
  );
  const [supabaseKey, setSupabaseKey] = useState(
    "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9idHdq...",
  );
    ...
    <div className="p-8 bg-slate-50 min-h-screen">
      ...
      <div className="mb-8">
        ...
        <p className="text-slate-600 mt-1">
          Administra variables globales, credenciales de base de datos y tu
          cuenta.
        </p>
      </div>
        ...
        <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 lg:col-span-2 space-y-6">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-1">
              Configura las credenciales de Supabase para almacenar y
          ...
          >
            <div className="flex items-center space-x-3">
              <span
                className={`w-3.5 h-3.5 rounded-full flex items-center justify-center ${isConnected ? "bg-emerald-500" : "bg-amber-500"}`}
              >
                <span
                  className={`w-2 h-2 rounded-full bg-white ${isConnected ? "animate-pulse" : ""}`}
                ></span>
              </span>
              <div>
                <p className="text-sm font-bold">
                  {isConnected
                    ? "Conectado al Backend Principal"
                    : "Ejecutando en Modo Aislado (Modo Mock)"}
                </p>
